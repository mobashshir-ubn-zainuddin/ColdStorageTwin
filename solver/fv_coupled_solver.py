...
class FVCoupledSolver:
    ...
    def __init__(self, mesh: Mesh):
        ...
        self.velocity_solver = VelocitySolver(mesh)
    ...
    def step(self, state: SimulationState, dt: float) -> Tuple[SimulationState, Dict[str, Any]]:
        ...
        """
        # 1. Read current state and compute properties
        ...
        # 4. Project Velocity (u^{n+1})
        u_new, v_new, w_new = self.velocity_solver.project_velocity(
            u_star, v_star, w_star, P_new, rho, dt
        )

        # 5. Solve Moisture Transport
        # Baseline: zero sources
        S_omega = np.zeros_like(omega)
        S_evap = np.zeros_like(omega)
        omega_trans, _ = self.moist_solver.solve_step(
            state, dt, S_omega, np.zeros_like(omega), S_evap
        )

        # 6. Apply Condensation
        # Use current T for saturation
        ...
        # 7. Solve Temperature Transport
        # Baseline: zero sensible sources
        S_Q = np.zeros_like(T)
        T_new = self.temp_solver.solve_step(
            state, dt, S_Q, S_latent
        )

        # 8. Update Simulation State
        # Create new state to avoid mutating old state during timestep
    ...
    def _compute_diagnostics(self, state: SimulationState, new_state: SimulationState, dt: float) -> Dict[str, Any]:
        ...
        # Simple divergence for diagnostic
        ...
        mass_residual = (rho_new - rho_old) / dt + div_rho_u
